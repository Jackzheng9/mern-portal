...
import { useGetUserByEmailMutation, useEditUserMutation, useSetUserPassMutation } from '../slices/userApiSlice';
...
const SetPassword = () => {
  ...
  const [ setUserPass,{isLoading, isError}] = useSetUserPassMutation();
  ...
  const formHandler = async (e) => {
    ...
    try {
      const apiData = await getUserByEmail(data).unwrap();
      console.log(apiData)
      setUserFound(true)
      toast.success("Email found! Please proceed to set password!")      
    } catch (error) {
      console.log(error)
